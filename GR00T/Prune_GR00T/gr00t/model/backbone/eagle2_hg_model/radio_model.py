...
def replace_vit_attn_with_flash_attn():
    if not FLASH_ATTN_AVAILABLE:
        warnings.warn("flash_attn is not installed; using the default PyTorch/timm attention path.")
        return
    if not torch.cuda.is_available():
        warnings.warn("CUDA is not available; using the default PyTorch/timm attention path.")
        return
