...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--context", default="QuantVLA benchmark")
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        print(
            f"[QuantVLA][ERROR] CUDA is not available for {args.context}. "
            "This run would fall back to CPU, so it is stopped.",
            file=sys.stderr,
        )
        print(
            "[QuantVLA][ERROR] Activate the GPU conda environment and check "
