#!/usr/bin/env python3
"""Shared utilities for QuantVLA benchmark scripts."""
...
def env_snapshot() -> dict[str, str]:
    ...
    for key, value in os.environ.items():
        if key in INTERESTING_ENV_KEYS or key.startswith(INTERESTING_ENV_PREFIXES):
            env[key] = value
...
def runtime_metadata(variant: str = "") -> dict[str, Any]:
    return {
        ...
        "duquant_env_enabled": duquant_env_enabled(),
        "atm_env_enabled": flag_enabled("GR00T_ATM_ENABLE"),
        "ohb_env_enabled": flag_enabled("GR00T_OHB_ENABLE"),
        "attn_implementation": os.environ.get("GR00T_ATTN_IMPLEMENTATION", "sdpa"),
        "env": env_snapshot(),
    }


def count_duquant_modules(model: torch.nn.Module) -> int:
    try:
        from gr00t.quantization import DuQuantLinear
    except Exception:
        return 0
    return sum(1 for module in model.modules() if isinstance(module, DuQuantLinear))


def duquant_storage_summary(model: torch.nn.Module) -> dict[str, int]:
    try:
        from gr00t.quantization import DuQuantLinear
    except Exception:
        return {}
    ...
    for module in model.modules():
        if not isinstance(module, DuQuantLinear):
            continue
        storage = str(getattr(module, "storage_mode", "unknown"))
        bits = int(getattr(module, "weight_bits", 0))
        key = f"{storage}_w{bits}"
