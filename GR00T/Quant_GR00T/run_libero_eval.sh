...
HOST=${QVLA_HOST:-localhost}

# Activate libero_test when running on the local conda setup. Docker images run
# directly in the image Python environment and do not contain conda.
if [[ "${QUANTVLA_SKIP_CONDA:-0}" != "1" && -f "$HOME/miniconda3/etc/profile.d/conda.sh" && -d "$HOME/miniconda3/envs/libero_test" ]]; then
    source "$HOME/miniconda3/etc/profile.d/conda.sh"
    conda activate libero_test
else
    echo "[QuantVLA] Conda env libero_test not found; using current Python environment"
fi
...
python run_libero_eval.py --task_suite_name "$TASK" --host "$HOST" --port 5556 "${EXTRA_ARGS[@]}"
