...
def format_memory_line(memory_summary):
    ...
    duquant_stats = memory_summary.get("server_duquant_linear_modules")
    if duquant_stats:
        parts.append(f"duquant_layers={duquant_stats['median_bytes']:.0f}")
    packed_stats = memory_summary.get("server_duquant_packed_modules")
    if packed_stats:
        parts.append(f"packed_layers={packed_stats['median_bytes']:.0f}")
    fake_stats = memory_summary.get("server_duquant_fake_modules")
    if fake_stats:
        parts.append(f"fake_layers={fake_stats['median_bytes']:.0f}")
    atm_stats = memory_summary.get("server_atm_env_enabled")
    if atm_stats:
        parts.append(f"atm_env={int(atm_stats['median_bytes'])}")
    ohb_stats = memory_summary.get("server_ohb_env_enabled")
    if ohb_stats:
        parts.append(f"ohb_env={int(ohb_stats['median_bytes'])}")
    return " | ".join(parts)
...
def format_realtime_memory_line(task_id, episode_idx, step, step_memory):
    duquant_layers = step_memory.get("server_duquant_linear_modules")
    packed_layers = step_memory.get("server_duquant_packed_modules")
    fake_layers = step_memory.get("server_duquant_fake_modules")
    atm_env = step_memory.get("server_atm_env_enabled")
    ohb_env = step_memory.get("server_ohb_env_enabled")
    parts = [
    ...
    if duquant_layers is not None:
        parts.append(f"duquant_layers={float(duquant_layers):.0f}")
    if packed_layers is not None:
        parts.append(f"packed_layers={float(packed_layers):.0f}")
    if fake_layers is not None:
        parts.append(f"fake_layers={float(fake_layers):.0f}")
    if atm_env is not None:
        parts.append(f"atm_env={int(float(atm_env))}")
    if ohb_env is not None:
        parts.append(f"ohb_env={int(float(ohb_env))}")
    return (
