...
def _add_latency_ms(timing: Optional[Dict[str, float]], key: str, elapsed_ms: float) -> None:
    if timing is not None:
        timing[key] = timing.get(key, 0.0) + elapsed_ms


def _timed_call(module: nn.Module, timing: Optional[Dict[str, float]], key: str, *args: Any, **kwargs: Any) -> Any:
    if timing is None:
    ...
    _sync_cuda_for_timing()
    _add_latency_ms(timing, key, (time.perf_counter() - start_time) * 1000.0)
    return output
...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        timing = getattr(self, "_latency_breakdown", None)
        ...
        if input_ids.shape[1] == 1:
            ...
            language_model_output = _timed_call(
                self.language_model,
                timing,
                "llm_forward_ms",
                input_ids=input_ids,
        ...
        # === Handle Multimodal Forward ===
            ...
            patch_features = _timed_call(self.vision_backbone, timing, "vision_backbone_ms", pixel_values)

            # Projection Logic =>> Update Attention Mask
            projected_patch_embeddings = _timed_call(self.projector, timing, "projector_ms", patch_features)
            ...
            if self.use_fastv:
                if hasattr(self.language_model, 'fastv_forward'):
                    if hasattr(self, "fastv_config") and isinstance(self.fastv_config, dict) and timing is not None:
                        self.fastv_config["_timing"] = timing
                    language_model_output = _timed_call(
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        self._latency_breakdown = {
            "preprocess_ms": 0.0,
            "vision_backbone_ms": 0.0,
            "projector_ms": 0.0,
            "llm_prefill_ms": 0.0,
            "token_pruning_ms": 0.0,
            "pruned_llm_generation_ms": 0.0,
            "llm_forward_ms": 0.0,
        ...
        _sync_cuda_for_timing()
        _add_latency_ms(
            self._latency_breakdown, "action_decode_ms", (time.perf_counter() - action_decode_start) * 1000.0
        )
