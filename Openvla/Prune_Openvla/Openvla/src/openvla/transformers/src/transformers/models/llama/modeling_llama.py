...
def _add_latency_ms(timing, key: str, elapsed_ms: float) -> None:
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def fastv_forward(
        ...
        timing = fastv_config.get("_timing") if isinstance(fastv_config, dict) else None
        if timing is not None:
            _sync_cuda_for_timing()
            llm_prefill_start = time.perf_counter()
        else:
            llm_prefill_start = None
        pruned_llm_start = None
            ...
            if use_cache:
                if hidden_states.shape[1] != 1:
                    if layer_idx != FASTV_k:
                        ...
                        if timing is not None and llm_prefill_start is not None:
                            _sync_cuda_for_timing()
                            _add_latency_ms(
                                timing, "llm_prefill_ms", (time.perf_counter() - llm_prefill_start) * 1000.0
                            )
                        pruning_start = time.perf_counter() if timing is not None else None
                        ...
                        if timing is not None and pruning_start is not None:
                            _sync_cuda_for_timing()
                            _add_latency_ms(
                                timing, "token_pruning_ms", (time.perf_counter() - pruning_start) * 1000.0
                            )
                            pruned_llm_start = time.perf_counter()
            else:
                if layer_idx<FASTV_k:
                    pruned_attention_mask = causal_mask
        ...
        if timing is not None:
            _sync_cuda_for_timing()
            if pruned_llm_start is not None:
                _add_latency_ms(
                    timing, "pruned_llm_generation_ms", (time.perf_counter() - pruned_llm_start) * 1000.0
                )
            elif llm_prefill_start is not None:
                _add_latency_ms(timing, "llm_prefill_ms", (time.perf_counter() - llm_prefill_start) * 1000.0)
