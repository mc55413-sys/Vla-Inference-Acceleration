...
from experiments.robot.openvla_direct_quant import DirectQuantConfig, quantize_openvla_language_model
    ...
    def __init__(
        ...
        if self.direct_quant_mode not in {None, "", "none", "fp16", "bf16"}:
            report = quantize_openvla_language_model(
                self.vla,
                DirectQuantConfig(mode=self.direct_quant_mode, group_size=direct_quant_group_size),
            )
            print(
                "[*] Direct quantization summary: "
                f"backend={report.backend}, replaced={report.replaced_linear_layers}, "
                f"model_size={report.original_model_size_mb:.2f}->{report.quantized_model_size_mb:.2f} MB"
            )
...
class DeployConfig:
    ...
    direct_quant_mode: str = "none"                                     # none | w8a16 | w8a8 | w4a16
