...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        ...
        if isinstance(past_key_values, tuple):
            dynamic_cache_cls = getattr(transformers, "DynamicCache", None)
            if dynamic_cache_cls is not None:
                past_key_values = dynamic_cache_cls.from_legacy_cache(past_key_values)

        if hasattr(past_key_values, "_seen_tokens") and hasattr(past_key_values, "crop"):
